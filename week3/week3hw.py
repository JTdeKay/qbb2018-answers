...
"""
Usage: ./week3hw.py <annotated vcf> 
"""
    ...
    if line[0] == "#":
        ...
        for var in info:
            if "DP=" in var:
                ...
                vdepth.append(float(depth2[0]))
            if "AF=" in var:
                freq = var.split("=")
                freq2 = freq[1].split(",")
                vfreq.append(float(freq2[0]))     
        for val in info2:
            vqual.append(float(info2[1]))
...
axes[1].hist(vfreq, bins = 800, color = "blue")
...
axes[2].hist(vqual, bins = 100, color = "green")
